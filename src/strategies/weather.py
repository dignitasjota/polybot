...
class WeatherConfig(StrategyConfig):
    ...
    def from_dict(cls, raw: dict, mode: str = "disabled") -> WeatherConfig:
        ...
        return cls(
            ...
            max_forecast_days=int(raw.get("max_forecast_days", 3)),
            min_edge=float(raw.get("min_edge", 0.08)),
            min_forecast_prob=float(raw.get("min_forecast_prob", 0.15)),
            min_agreement=float(raw.get("min_agreement", 0.20)),
            max_price=float(raw.get("max_price", 0.75)),
            max_bet_per_trade=float(raw.get("max_bet_per_trade", 10.0)),
            bankroll=float(raw.get("bankroll", 200.0)),
            kelly_multiplier=float(raw.get("kelly_multiplier", 0.25)),
            max_bets_per_cycle=int(raw.get("max_bets_per_cycle", 3)),
