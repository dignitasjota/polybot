...
**[CORREGIDO Jul 11, 2026] C3 — Protección anti-apuesta-cruzada muerta (enum vs string)**
`base.py:85` — filtraba `status in ("pending", "confirmed")` pero `TradeRecord.status` es `OrderStatus` (Enum); la comparación siempre daba `False`. `get_open_positions()` devolvía siempre `[]` → `has_opposite_position` nunca bloqueaba → dos estrategias podían comprar YES y NO del mismo mercado. Idem `count_paper_trades_today` y `count_active_positions`.
> Fix: helper `_status_str()` normaliza el status (enum→`.value`, o string tal cual) y `get_open_positions` compara contra `_OPEN_STATUSES` = {pending, live, matched, mined, confirmed}. Se amplió el conjunto respecto al `pending/confirmed` original porque en live una orden `live`/`matched` también es posición abierta que la protección debe ver. Reactiva las tres protecciones a la vez. Tests: `test_base_context.py` (13, casos enum y string).

**[CORREGIDO Jul 11, 2026] C4 — Completeness live: `break` deja pata aceptada sin cancelar ni unwind**
`completeness_scanner.py:649,659` — el bucle hacía `break` al primer fallo; si la primera pata falló al colocarse pero la segunda fue aceptada, su ID no se recopilaba → orden viva → fill sin hedge con `cost_total = 0` (invisible).
> Fix: `order_ids` se mantiene **alineado con `token_ids`** (`""` para la pata que falló al colocarse) y se pasa SIEMPRE por el pipeline de fill verification (`_poll_fills` → cancel resting → `_unwind_leg`) salvo que **nada** se colocara (`not any(order_ids)` → failed cost=0). Una pata con id `""` puntúa `matched=0`, forzando `pair<min_shares` → unwind de la pata aceptada. `_poll_fills` salta ids vacíos. Tests: +2 en `TestFillVerification`.

**[CORREGIDO Jul 11, 2026] C5 — Executor (directional/copy) con el bug de "fills invisibles"**
`executor.py:1244,1284-1287` — `success: true` con `orderID` vacío (FAK match instantáneo) se trataba como `order_rejected`; una excepción tras `post_order` marcaba `FAILED` sin verificar. Portado de weather/completeness (el bug costó $44.5 en weather).
> Fix: `_parse_post_order_response()` (fill si `orderID` **o** `tradeIDs` **o** `success`); FAK match → `MATCHED` (ya llenó, sin monitoreo), `orderID` → `LIVE` (resting, monitoreado). En el `except`, `_verify_fill_via_api()` consulta la Data API (`_fetch_user_fills_via_api`, funder + session efímera) antes de rendirse → `MATCHED` si la orden vive on-chain. Nueva constante `DATA_API`. Tests: `test_executor_fills.py` (10).
